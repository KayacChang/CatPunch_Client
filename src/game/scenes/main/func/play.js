...
import {wait, clone, times, ceil, log, table} from '@kayac/utils'
...
export function play (scene) {
    const {slot, energy, neko, freeSpinIcon, normalTable, freeGameTable} = scene

    const coinPos = slot.view.children.filter(({name}) => name.includes('Coin'))
    ...
    app.on('GameResult', async (result) => {
        ...
        const freeWin = result.hasFreeGame
            ? result.freeGame.reduce((a, b) => a.scores + b.scores)
            : 0
        const totalWin = normalWin + reSpinWin + freeWin

        await spin(scene, slot.reels, result.normalGame)
        ...
        if (result.hasReSpin) {
            ...
            slot.view.getChildByName('FXReel_M').anim.visible = true

            app.sound.play('maybeBonus')

            reSpinEffect(scene)

            await spin(scene, [slot.reels[1]], result.reSpinGame)
        ...
        if (energy.scale === 10) {
            ...
            for (const result of freeGame) {
                table(result)
                const multiply = freeGame.indexOf(result) + 1

                freeGameEffect(scene, multiply)

                slot.view.children
                    .filter(
                        ({name}) => name === 'FXReel_L' || name === 'FXReel_R',
                    )
    ...
    async function showScores (scores) {
        ...
        const coinEffect = slot.reels.map((reel) => {
        ...
        const effect = scoresEffect(scene, scores)

        await Promise.all([...coinEffect, effect])
    }
